...
to an argument, the number literal $3$. The syntax here is all defined
formally in Section \ref{sec:hazel}, but should be intuitive.

The example proceeds by tracing what happens to related terms as different
...
are H-Expressions with focus on working on filling just one of the
holes. The third column names the action that is performed to the hole in
focus in the Z-Expression to produce the next line, which may or may not
produce a substantively different H-Expression.
%
Lines 1--10 consist of the user interactively constructing the identity
function. After the break, lines 11--15 consist of the user editing this
