...
\section{Hazelnut}
In this section, we define {Hazelnut}, a minimal bidirectionally typed structure editor based on the simply-typed lambda calculus extended with a single base type, $\tnum$, equipped (for simplicity) with a single operation, addition.

The main constituents of Hazelnut are:
\begin{itemize}
\item \textbf{H-types} and \textbf{H-expressions} (Sec. \ref{sec:holes}). These are terms that contain \emph{holes}, which mark subterms that are ``under construction.'' H-types classify H-expressions according to a \emph{bidirectionally typed} static semantics.
...
\item \textbf{Actions} (Sec. \ref{sec:actions}). These move the cursor or modify the subtree marked by the cursor.

The action model is defined in a bidirectional manner such that whenever an action is performed on a well-typed tree, it produces another well-typed tree in a \emph{sensible} manner (we define \emph{sensibility} in Sec. \ref{sec:actions}.)
\end{itemize}
