...
So far, we have assumed that only complete expressions have a dynamic semantics. It may be possible to give a dynamics to incomplete expressions. Prior work on staged evaluation suggests that there may be a connection to modal logic, viewing holes as quantifying over other worlds \cite{DBLP:journals/jacm/DaviesP01}. In developing a dynamic semantics, we will need to handle terms like 
$\hhole{\hhole{e}}$. Intuitively, this term should be considered equivalent to $\hhole{e}$. We have not yet
...
We thank Ed Morehouse for helpful discussions.
