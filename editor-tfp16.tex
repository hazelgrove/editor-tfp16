...
\begin{array}{|c|c||l|l|}
\hline
\textbf{H-Expression} & \textbf{Z-Expression} & \textbf{Next Action} & \textbf{Semantic Rule}
\\
\hline
\hhole{} &
\zwsel{\hhole{}}
...
\\
\hapP{\hlam{x}{{x}} : \tarr{\tnum{}}{\tnum{}}}{\hhole{3}}
&
\hapP{\hlam{x}{{x}} : \tarr{\tnum{}}{\tnum{}}}{\zwsel{\hhole{3}}}
&
\aFinish{} & ?
\\
\hapP{\hlam{x}{{x}} : \tarr{\tnum{}}{\tnum{}}}{{3}}
&
\hapP{\hlam{x}{{x}} : \tarr{\tnum{}}{\tnum{}}}{\zwsel{{3}}}
&
\quad\textrm{---{}---}
&
\quad\textrm{---{}---}
...
\end{array}
\]
\caption{Example of constructing the identity function interactively in Hazelnut.}
...
The statics of Hazelnut is organized as a \emph{bidirectional type system} \cite{bidi}, i.e. around the following mutually defined typing judgements:
...
\end{array}\]
where typing contexts, $\hGamma$, map variables $x \in \domof{\hGamma}$ to hypotheses $x : \htau$. 

Algorithmically, the type analysis judgement specifies a function where the type is an input, and the type synthesis judgement specifies a function where the type is an output. This provides \emph{local type inference}. Moreover, making a judgemental distinction between analysis and synthesis is essential for giving a sensible action semantics to our system (Sec. \ref{sec:actions}.)
...
Type synthesis is stronger than type analysis, i.e. if an expression is able to synthesize a type, it can also be analyzed against that type. This is expressed by the following \emph{subsumption rule}:
...
\end{equation}
The \emph{H-type compatibility} judgement, $\tcompat{\htau}{\htau'}$, reduces to syntactic equality for complete H-types. For incomplete H-types, the rules are given after we discuss the semantics of holes below. First, we will reproduce the rules for the standard constructs. 

Type ascription allows the user to explicitly annotate an expression with the type that it is to be analyzed against:
...
In a function application, the function must synthesize an arrow type. The argument is analyzed against the synthesized argument type.
...
Non-empty holes contain H-expressions that are ``under construction''. This expression must synthesize a type, but because this type may change, the non-empty hole also synthesizes the hole type:
...
\end{equation}
\end{subequations}
Consequently, by subsumption, we can derive that $\hana{\hGamma}{\hhole{\hnum{42}}}{\tarr{\tnum}{\tnum}}$.
