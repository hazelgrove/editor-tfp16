...
\begin{abstract}
Programs are rich inductive structures, yet today, programmers  interact with them mainly through ``flattened'' textual representations. This structural mismatch can impose a non-trivial cognitive burden on programmers.
...
\emph{Structure editors} are program editors where each edit action corresponds to a sensible structural  transformation. 
...
2) a \emph{selection model}; and 3) an \emph{action model}. 
This system is intended to be foundational. 
In particular, we articulate our vision of a full-scale programming system designed ``from the ground up'' around the same fundamental concepts as this calculus.
...
%
Programs are rich inductive structures. 
Unfortunately, this fact is somewhat obscure because  programmers typically construct and interact with  programs only indirectly, using a text editor composed with a parser. 
There are some benefits to this approach, to be sure, but the structural mismatch between programs and text  also imposes a cognitive burden. 
For example, the primitive edit actions available in a text editor (e.g. inserting or deleting a character or word)  do not always correspond to  sensible structural transformations. 
